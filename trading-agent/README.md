...
└── agent/
    ...
    └── strategy/
        ...
        ├── mean_reversion.py # 均值回歸(低買高賣,跌了買漲了賣)
...
python3 compare.py --regime all           # 策略 × 行情 報酬矩陣(追漲殺跌 vs 低買高賣)
...
## 追漲殺跌 vs 低買高賣:兩種相反的信仰

看 `sma_cross` 的交易紀錄會覺得奇怪:它常常「賣在比買進更低的價格」。
那不是 bug,是**趨勢跟隨**的核心設計——認錯出場。兩個家族的信仰是相反的:
...
| 漲的時候 | 買進 / 續抱 | 賣出獲利 |
| 跌的時候 | 認賠出場 | 加碼買進 |
| 本骨架的實作 | `sma_cross`、`rule_stack` | `grid`、`mean_reversion` |
| 死法 | 震盪盤被上下巴來巴去 | 單邊下跌一路接刀 |

哪個對?**要看行情。** `compare.py --regime all` 直接把答案算出來
(每格是 5 組亂數的平均報酬):

| 策略 | 多空交替 | 震盪盤 | 單邊上漲 | 單邊下跌 |
|---|---|---|---|---|
| 買進抱著(=行情本身) | +16.6% | +1.2% | +73.8% | -21.8% |
| grid(低買高賣) | +1.3% | **+3.8%** | +1.6% | -7.5% |
| sma_cross(追漲殺跌) | -2.9% | -15.2% | **+33.6%** | -19.3% |
| mean_reversion(低買高賣) | +2.3% | +2.5% | +2.8% | **-1.8%** |
| mean_rev_nostop(不停損) | +1.9% | +2.5% | +3.0% | -4.1% |
| rule_stack(追漲殺跌) | +4.9% | -19.5% | **+47.5%** | -17.2% |

三個重點:

1. **震盪盤是低買高賣的主場**:`grid` +3.8%,而追漲殺跌的 `rule_stack` -19.5%
   ——價格上下來回,追漲殺跌每次都追在高點、殺在低點,被巴到懷疑人生。
2. **單邊上漲是趨勢跟隨的主場**:`rule_stack` +47.5%,而低買高賣只吃到 +2.8%
   ——因為它一漲就獲利了結,然後眼睜睜看著行情繼續走,再也等不到「跌回來」。
3. **停損的價值在下跌行情才看得到**:`mean_reversion`(有停損)-1.8%,
   拿掉停損變成 -4.1%;而越跌越買的 `grid` 是 -7.5%。

所以「跌了買、漲了賣」不是更聰明的做法,只是**押注在不同的行情假設上**。
真正該問的不是「哪個策略好」,而是「我面對的是哪種行情,猜錯時賠得起嗎」。
